...
# -*- coding: utf-8 -*-
"""
Misi Navigasi Segmen: WP3 -> WP4
Berputar (Yaw) di tempat terlebih dahulu, lalu maju menuju WP4.
"""
...
LOCK_TOLERANCE = 40
STABLE_DURATION = 3.0
TARGET_ID = 4  # Target ArUco ID untuk WP4
    ...
    try:
        while True:
            ...
            if mode != "GUIDED":
                ...
                if state == STATE_INIT:
                    print("✅ Mode GUIDED aktif. Memulai ROTASI YAW ke WP4.")
                    ...
                    if cur_yaw is not None:
                        ...
                        if diff < 5.0:
                            print("✅ Rotasi selesai. Maju ke GPS WP4...")
                    ...
                    if cur_lat and cur_lon:
                        ...
                        cv2.putText(display_frame, f"Dist WP4: {dist:.1f} m", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                        if dist < 2.0:
                            if use_aruco:
                                print("✅ Mendekati WP4. Beralih ke pencarian ArUco!")
                                state = STATE_CENTER_ARUCO
                            else:
                                print("✅ Mendekati WP4. Verifikasi ArUco DINONAKTIFKAN. SELESAI SEGMEN INI.")
                                state = STATE_DONE
                        else:
                            goto_gps_position(master, wp_target['lat'], wp_target['lon'], target_alt)

                elif state == STATE_CENTER_ARUCO:
                    state_str = "VISUAL CENTERING WP4"
                    if ids is not None and TARGET_ID in ids:
                        ...
                        if is_locked:
                            if stable_start_time == 0: stable_start_time = time.time()
                                ...
                                print("✅ ArUco WP4 Verified! SELESAI SEGMEN INI.")
                                state = STATE_DONE
                                stable_start_time = 0
            ...
            # cv2.imshow("Navigasi WP3->WP4", display_frame) # Headless mode
