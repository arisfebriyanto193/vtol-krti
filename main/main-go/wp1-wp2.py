...
def main():
    ...
    wp_target = config.get('waypoints', {}).get('wp2', {})
    ...
    state = STATE_INIT
    stable_start_time = 0
    cur_lat, cur_lon, cur_alt, cur_yaw = None, None, None, None

    print("\n🚀 Menunggu mode GUIDED untuk memulai rotasi ke WP2.")
    
    try:
        while True:
            t_lat, t_lon, t_alt, t_yaw = get_telemetry(master)
            if t_lat is not None: cur_lat = t_lat
            if t_lon is not None: cur_lon = t_lon
            if t_yaw is not None: cur_yaw = t_yaw
            ...
            if mode != "GUIDED":
                ...
                if state == STATE_INIT:
                    print("✅ Mode GUIDED aktif. Memulai ROTASI YAW ke WP2.")
                    ...
                    if cur_yaw is not None:
                        ...
                        if diff < 5.0: # Toleransi 5 derajat
                            print("✅ Rotasi selesai. Maju ke GPS WP2...")
                            state = STATE_GOTO_GPS
                        else:
                            # Terus kirim perintah rotasi jika belum sampai
                            rotate_to_yaw(master, wp_target['yaw'])
            ...
            cv2.imshow("Navigasi WP1->WP2", display_frame)
